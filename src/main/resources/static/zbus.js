...
function ajaxInvoke(msg, beforeSend){
    if(beforeSend){
        beforeSend(msg);
    }
...
class RpcProcessor {
    ...
    addModule(module, objectOrFunc, target, docEnabled) {   
    ...
    enableDoc(value, docModule = "") {
        if (!value) return;  
        var info = new RpcInfo(this);
        this.addModule(docModule, info.index, info, false);  
    }
...
class RpcServer {
    ...
    start() {
        ...
        for (var i = 0; i < this.clientCount; i++) {
            var client = new MqClient(this.address);
            client.onopen = () => {
                var msg = {};
                msg.headers = {
                    cmd: 'create',
            ...
            client.addMqHandler(this.mq, this.channel, (req) => {
                ...
                var url = req.url;
                var mqPrefix = "/"+this.mq;
                if(url && url.startsWith(mqPrefix)){
                    url = url.substr(mqPrefix.length);
                    url = joinPath("/", url);
                    req.url = url;
                }
...
class RpcInvoker {
    ...
    invoke() {
        ...
        if (typeof (req) == 'string') {
            ...
            msg = new Message();
            msg.url = joinPath(this.module, req);
            msg.body = params;  
        ...
        return this.client._invoke(msg, this.beforeSend);
    } 
    ...
    proxyMethod(method) {
        ...
        return function () {
            ...
            msg.url = joinPath(invoker.module, method); 
            msg.body = params;
...
class RpcClient {
    constructor(address, app) {
        ...
        this.proxy = new Proxy(this, {
            get: function (target, name) {
                return name in target ? target[name] : client.module(name, client.defaultApp);
            }
        });
        this.defaultModule = this.module("", this.defaultApp);
        return this.proxy;
    }  

    module(module, mq){
        const invoker = new RpcInvoker(this);
    ...
    invoke() {
        return this.defaultModule.invoke(...arguments);
    }

    _invoke(req, beforeSend) {
        if(this.wsClient){
            return this._wsInvoke(req, beforeSend);
        } else {
            return ajaxInvoke(req, beforeSend);
        }
    } 

    _wsInvoke(req, beforeSend) {
        var p;
        if (!this.wsClient.active()) {
            p = this.wsClient.connect().then(() => {
                return this.wsClient.invoke(req, beforeSend);
            });
        } else {
            p = this.wsClient.invoke(req, beforeSend);
        }
